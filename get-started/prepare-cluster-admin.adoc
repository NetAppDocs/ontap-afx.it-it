...
NOTE: A partire da ONTAP 9.19.1, gli amministratori SVM possono monitorare e gestire i propri SVM utilizzando una dashboard dedicata di System Manager. Scopri di più su https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager dashboard amministratore SVM"^]
...
Esistono diverse limitazioni, tra cui valori minimi e massimi, imposte da AFX durante la configurazione e l'utilizzo del cluster. Tali limitazioni rientrano in diverse categorie.


CAUTION: Prima di aumentare un parametro operativo del sistema storage AFX, è necessario valutare come potrebbe influire sulle applicazioni e sull'intero cluster. Ad esempio, mentre l'espansione del numero di nodi controller aumenta la capacità complessiva, potrebbe anche aumentare il traffico nella rete del cluster. È importante comprendere le implicazioni di queste modifiche di configurazione e come potrebbero influire sull'ambiente.

Nodi controller per cluster:: Ogni cluster AFX deve avere almeno quattro nodi. Il numero massimo di nodi varia in base alla release di ONTAP; ad esempio, la release 9.19.1 RC consente fino a 32 nodi per cluster AFX.
...
Interruttori a grappolo:: Sono necessari almeno due switch nella rete di archiviazione del cluster.  Il numero massimo consentito è determinato in base al numero totale di nodi controller nel cluster.


È consigliabile consultare i dettagli disponibili nello NetApp Hardware Universe e nell'Interoperability Matrix Tool per determinare le funzionalità del cluster AFX. Consultare anche link:../release-notes/changes-to-limits-defaults.html["Modifiche ai limiti e alle impostazioni predefinite di ONTAP"] nella sezione delle note di rilascio di AFX.
